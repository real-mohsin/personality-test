...
function PersonalityTest() {
  ...
  const nextQuestionHandler = (isCorrect: boolean): void => {
    if (isCorrect === true) {
      setScore(score + 1)
    }
    ...
    if(score >2) {
      setIntro('You are Introvert')
    }
    else {
      setIntro('You are Extrovert') 
    }
    ...
    return (
      ...
      {result ? (
        <div className="container">
          <div className='txt-center result'>
            {intro}
          </div>
        </div>
      ): (
        <div className='container'>
          <div className='question-number txt-center'>{currentQuestion + 1}/{questions.length}</div>
          <div className='question txt-center'>
            {questions[currentQuestion].questionText}
          </div>
          <div className='options'>
            {questions[currentQuestion].questionOptions.map((questionOption)=><button className='question-options' onClick={()=>nextQuestionHandler(questionOption.isCorrect)}>{questionOption.answerText}</button>)}
          </div>
        </div>
      )}
